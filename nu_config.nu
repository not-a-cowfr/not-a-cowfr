...
# this is so buggy its not even funny, im pretty sure its _mostly_ the xargs command's fault
# def "git total lines" [
#     --include (-i): string, # Only include a certain file type, eg. --include .nu or -i rs
#     --exclude (-e): string, # Exclude a certain file type
#     --exclude-bloat (-b): string, # Exclude files known to be autogenerated or bloat that shouldnt count as lines of code, eg. config files
...
#     let lines = (git ls-files | xargs wc -l err> nul | lines | each { |line| $line | str trim -l } | where $it =~ '^\d+\s.+\..+')

#     let stats = $lines
#         | split column " " lines file
#         | each { |row| { file: $row.file, lines: ($row.lines | into int) } }

#     print $"\n(ansi white_bold)Total: (ansi reset)(ansi green)($stats | where file == "total" | get lines | first)(ansi reset)\n"
